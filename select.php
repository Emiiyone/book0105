...
if ($status==false) {
  ...
  while( $result = $stmt->fetch(PDO::FETCH_ASSOC)){
    ...
    // ログインしている場合のみ更新ボタンを表示
    if(isset($_SESSION['chk_ssid'])) {
        $view .= "<div class='btns'>";
    // 更新ボタン
    $view .= "<div class='upBtn'><a class='updateBtn' data-id='" . h($result['id']) . "' href='#'>Edit</a></div>";
    $view .= "</div>";
    }
    //$view .= "<div class='upBtn'><a class='updateBtn' data-id='" . h($result['id']) . "' href='#'>Edit</a></div>";
    //$view .= "</div>";
    ...
    $view .= "<div class='ups'>";
    $view .= "<div class='submitUp'><button type='submit' class='submitUpdate' data-id='" . h($result['id']) . "' onclick='return confirmDelete()'>Update</button></div>";
    // 削除ボタン
    if ($_SESSION['kanri_flg'] === 1){
        $view .= "<div class='deleteBtnWrap'><a class='deleteBtn' data-id='" .h($result['id']) . "' href='#'>Delete</a></div>";
      }
    //$view .= "<div class='deleteBtnWrap'><a class='deleteBtn' data-id='" .h($result['id']) . "' href='#'>Delete</a></div>";
    $view .= "<div class='closeUp'><button class='close'>Close</button></div>";
    $view .= "</div>";
    $view .= "</div>";
    $view .= "</div>";
    $view .= "</div>";
